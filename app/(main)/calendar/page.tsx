...
import { addDays, format, isToday, startOfISOWeek, endOfISOWeek, isSameWeek, subDays } from 'date-fns';
import { es } from 'date-fns/locale';
import { cn } from '@/lib/utils';
import { CalendarGrid } from '@/components/calendar/CalendarGrid';
import { Button } from '@/components/ui/button';
import { ChevronLeft, ChevronRight, Plus, CalendarDays, List, Sparkles } from 'lucide-react';
...
export default function CalendarPage() {
  ...
  const isCurrentWeek = isSameWeek(currentDate, new Date(), { weekStartsOn: 1 /* Lunes */ });
  ...
  const handleDragEnd = async (postId: string, newStartTime: Date) => {
    const updatedPost = await postService.updatePost(postId, { startTime: newStartTime });
    if (updatedPost) {
    ...
    <>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2">
                    <Switch id="optimal-hours" checked={showOptimalHours} onCheckedChange={setShowOptimalHours} />
                    <Label htmlFor="optimal-hours" className="flex items-center gap-1 text-sm">
                ...
                <Select defaultValue='week'>
                    ...
                    <SelectContent>
                        ...
                        <SelectItem value="month" disabled>Mes</SelectItem>
                    </SelectContent>
                </Select>
                <ToggleGroup type="single" defaultValue="calendar" variant="outline">
                    <ToggleGroupItem value="calendar" aria-label="Vista de calendario"><CalendarDays className="h-4 w-4" /></ToggleGroupItem>
                    <ToggleGroupItem value="list" aria-label="Vista de lista" disabled><List className="h-4 w-4" /></ToggleGroupItem>
                </ToggleGroup>
      ...
      <div className="mt-4 flex-grow rounded-lg shadow-sm border flex flex-col overflow-hidden">
        {isLoading ? (
          ...
          <>
            {/* Day Headers */}
            <div className="grid grid-cols-[5rem_repeat(7,minmax(0,1fr))] shrink-0 border-b" style={{ scrollbarGutter: 'stable' }}>
              <div className="border-r bg-background"></div> {/* Empty corner */}
              {days.map(day => (
                  <div key={day.toString()} className="flex flex-col items-center justify-center h-16 text-center border-r bg-background">
                      <h3 className={cn("text-sm font-semibold uppercase mb-1", isToday(day) && 'text-primary')}>{format(day, 'E', { locale: es })}</h3>
                      {isToday(day) ? (
                      <div className="bg-primary text-primary-foreground rounded-full h-7 w-7 flex items-center justify-center text-sm font-bold">
                          {format(day, 'd')}
                      </div>
                      ) : (
                      <p className='text-xl font-medium text-muted-foreground'>{format(day, 'd')}</p>
                      )}
                  </div>
              ))}
            </div>

            <div className="flex-grow overflow-auto" style={{ scrollbarGutter: 'stable' }}>
                {/* Calendar Body */}
                <CalendarGrid 
                posts={posts} 
